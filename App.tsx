...
const App: React.FC = () => {
  ...
  const createHash = async (data: string): Promise<string> => {
    ...
    return hashHex.slice(0, 16); // Truncate for display
  };
  ...
  const handleFetchData = async () => {
    ...
    try {
      ...
      const currentHash = await createHash(JSON.stringify(newBlockData) + previousHash);
      ...
      <div className="container mx-auto p-4 md:p-8">
        <header className="text-center mb-12">
          ...
          <p className="text-gray-400 mt-2 text-lg">
            A decentralized approach to weather data reporting.
          </p>
        </header>
          ...
          <section className="lg:col-span-2">
            {blocks.length === 0 && !isLoading && (
              <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-gray-800/50 rounded-lg">
                ...
                <p className="text-gray-400 mt-2">Select a country and click 'Add New Weather Block' to start the chain.</p>
              </div>
            )}
